...
class FullConversationAppDogfoodTest(unittest.TestCase):
    ...
    def test_full_conversation_dogfood_generates_app_and_holistic_review(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(payload["lifecycle"]["stage_count"], 11)
            self.assertEqual(payload["lifecycle"]["final_stage"], "analysis")
            self.assertEqual(payload["lifecycle"]["final_stage_state"], "approved")
            self.assertEqual(len(payload["lifecycle"]["approved_stages"]), 11)
            self.assertEqual(payload["lifecycle"]["artifact_file_count"], 12)
            self.assertEqual(payload["conversation_review"]["turn_count"], 11)
            self.assertEqual(payload["conversation_review"]["event_count"], 88)
            ...
            self.assertTrue((output_dir / "lifecycle-artifacts" / "07-deployment" / "artifacts" / "deployment-proof.md").exists())
            ...
            generated_js = (output_dir / "generated-app" / "src" / "app.js").read_text(encoding="utf-8")
            self.assertNotIn("innerHTML", generated_js)
            self.assertIn("textContent", generated_js)

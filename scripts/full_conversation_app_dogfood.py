...
def public_transcript_markdown() -> str:
    """Return the deterministic public transcript used by this dogfood.

    The runtime export also includes HTML and JSONL event streams with generated
    timestamps/event IDs. This markdown keeps the owner/Hermes exchange stable
    and reviewable in GitHub while preserving the same scripted messages the
    dogfood records at runtime.
    """
    ...
    for index, stage_id in enumerate(runtime.stage_ids(), start=1):
        stage = runtime.stage_by_id(stage_id)
        script = STAGE_SCRIPT[stage_id]
        artifact_path = f"apps/{APP_ID}/lifecycle/{stage.directory}/artifacts/{stage_id}-proof.md"
        artifact_mirror_path = f"artifacts/full-conversation-app-dogfood/lifecycle-artifacts/{stage.directory}/artifacts/{stage_id}-proof.md"
        lines.extend(
            [
                "",
                f"### {index}. {runtime.owner_stage_label(stage_id)}",
                "",
                f"- Stage ID: `{stage_id}`",
                f"- Runtime proof artifact: `{artifact_path}`",
                f"- Committed artifact mirror: [`{artifact_mirror_path}`]({artifact_mirror_path})",
                "",
                "**Owner:**",
                "",
                script["owner"],
                ...
                "",
                f"- Rationale summary: {script['title']} is ready for owner review with linked proof artifact.",
                "- Gate checks: foundation gate passed; stage artifact present; owner approval required.",
                f"- Artifact refs: `{artifact_path}`; committed mirror `{artifact_mirror_path}`.",
                f"- State transition: `{stage_id}` collecting -> ready_for_review.",
                f"- Next action: Owner reviews {runtime.owner_stage_label(stage_id)} evidence and approves or requests revision.",
            ]
        )
        if stage_id == "engineering":
            lines.append("- Generated source refs: `index.html`, `src/app.js`, `src/styles.css`, `public/config.json`, `README.md`.")
        if stage_id == "qa":
            lines.append("- QA proof ref: `apps/pocket-orchard/lifecycle/06-qa/artifacts/qa-checks.json`; committed mirror [`artifacts/full-conversation-app-dogfood/lifecycle-artifacts/06-qa/artifacts/qa-checks.json`](artifacts/full-conversation-app-dogfood/lifecycle-artifacts/06-qa/artifacts/qa-checks.json).")
        if stage_id in FUTURE_CREDENTIAL_STAGES:
            lines.append("- Credential gate: real external credentials are absent and explicitly owner-deferred for this local proof.")
    lines.extend(
        [
            "",
...
def write_public_transcript(path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(public_transcript_markdown(), encoding="utf-8")
...
def run(output_dir: Path) -> dict[str, Any]:
    ...
    lifecycle_artifacts_output = output_dir / "lifecycle-artifacts"
    if generated_app_output.exists():
        shutil.rmtree(generated_app_output)
    if lifecycle_artifacts_output.exists():
        shutil.rmtree(lifecycle_artifacts_output)
    ...
    with tempfile.TemporaryDirectory(prefix="weave-full-conversation-") as tmpdir:
        ...
        shutil.copytree(root / "apps" / APP_ID / "lifecycle", lifecycle_artifacts_output)
        ...
        lifecycle_artifact_checksums = {
            str(path.relative_to(lifecycle_artifacts_output)): sha256(path)
            for path in sorted(lifecycle_artifacts_output.rglob("*"))
            if path.is_file()
        }
        ...
        return {
            ...
            "lifecycle": {
                ...
                "artifact_dir": str(lifecycle_artifacts_output),
                "artifact_file_count": len(lifecycle_artifact_checksums),
                "artifact_checksums": lifecycle_artifact_checksums,
            },
            "qa": qa_result,
            "conversation_review": {
...
def main() -> int:
    ...
    parser.add_argument("--transcript-out", type=Path, default=None, help="Path for the stable public markdown transcript.")
    ...
    if args.transcript_out is not None:
        write_public_transcript(args.transcript_out)
        report["public_transcript_path"] = str(args.transcript_out)

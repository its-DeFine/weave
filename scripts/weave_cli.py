...
COS_FIRST_RUN_ONBOARDING_QUESTIONS = [
    "What should I call you, and what partner behavior is useful: concise executor, careful planner, or frequent reviewer?",
...
def print_cos_bootstrap_payload(payload: dict[str, object], output: TextIO, *, as_json: bool) -> None:
    ...
    if "inferred_lifecycle_stage" in payload:
        print_line(output, f"- inferred_lifecycle_stage: {payload['inferred_lifecycle_stage']}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    tui_parser.add_argument("--cockpit", action="store_true", help="open the resumable lifecycle cockpit without running executor steps")
    tui_parser.add_argument("--view", choices=weave_tui.COCKPIT_VIEWS, default="overview", help="cockpit view to render")
    tui_parser.add_argument("--resume", action="store_true", help="restore the last saved cockpit view for this app")
    tui_parser.add_argument("--loop", action="store_true", help="force the cockpit command loop even when stdin/stdout are not TTYs")
    tui_parser.add_argument("--once", action="store_true", help="render one cockpit frame and exit")

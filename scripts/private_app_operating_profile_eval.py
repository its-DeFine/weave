...
OUTPUT_ROOT_MARKER = ".weave-private-app-eval-output-root"
...
def is_safe_temp_eval_root(path: Path) -> bool:
    """Return true for explicitly named temp eval roots used by review runs."""
    temp_root = Path(tempfile.gettempdir()).resolve()
    if not path_is_relative_to(path, temp_root):
        return False
    lowered_parts = [part.lower() for part in path.parts]
    name = path.name.lower()
    return (
        name.startswith("weave-")
        or name.startswith("private-app-")
        or "private-app-operating-profile-evals" in lowered_parts
    )


def force_delete_output_root(output_root: Path) -> Path:
    """Remove an eval output root only when it is clearly safe to delete.

    `--force` is convenient for generated evidence, but a bare `shutil.rmtree` on
    user-selected paths can erase unrelated work. This guard permits deletion of
    empty directories, roots previously marked by this harness, or explicitly
    named temporary eval roots. Everything else requires manual cleanup or a fresh
    output directory.
    """
    ...
    if resolved in protected_roots:
        raise ValueError(f"--force refused to delete protected output root: {resolved}")
    if output_root.is_symlink() or resolved.is_symlink():
        raise ValueError(f"--force refused to delete symlink output root: {output_root}")
    if not resolved.exists():
        return resolved
    if not resolved.is_dir():
        raise ValueError(f"--force refused to delete non-directory output root: {resolved}")

    marker = resolved / OUTPUT_ROOT_MARKER
    is_empty = not any(resolved.iterdir())
    if not (marker.is_file() or is_empty or is_safe_temp_eval_root(resolved)):
        raise ValueError(
            "--force refused to delete a non-empty unmarked output root. "
            f"Use a fresh --output-dir, manually inspect/remove it, or rerun against "
            f"a harness-created root containing {OUTPUT_ROOT_MARKER}."
        )
...
def run_eval(output_root: Path, scenarios: list[dict[str, Any]], parallel: int) -> dict[str, Any]:
    ...
    write_text(
        output_root / OUTPUT_ROOT_MARKER,
        "Generated by scripts/private_app_operating_profile_eval.py. This root is safe for guarded --force cleanup.",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Remove an existing output directory before running, guarded to marked/empty/temp eval roots",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.force:
        try:
            output_root = force_delete_output_root(output_root)
